...
def parse_args():
    ...
    parser.add_argument("--work_dir", help="the dir to save logs and tracking results", default='./res')
    parser.add_argument(
...
def filter(preds):
    preds_filtered = []
    for p in preds:
        valid = p['detection_name'] == 'pedestrian' and p['detection_score'] > 0.1 \
            or (p['detection_name'] != 'pedestrian' and p['detection_score'] > 0.025)
        if valid:
